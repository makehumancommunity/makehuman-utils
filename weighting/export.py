...
def exportVertexGroups(scn, ob, filepath):
    ...
    for vg in vgroups:
        ...
        if weights:
            list.append((vgname, weights))

    filename = os.path.expanduser(filepath)
    io_json.saveJson(list, filename, maxDepth=0)
    """
    fp = open(filename, "w")
...
class VIEW3D_OT_ExportVertexGroupsButton(bpy.types.Operator):
    ...
    def execute(self, context):
        scn = context.scene
        exportVertexGroups(scn, context.object, scn.MhxVertexGroupFile)
        return{'FINISHED'}
